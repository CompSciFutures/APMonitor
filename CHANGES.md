...
On-premises LAN monitoring tool with guaranteed alerts.
...
- Added a timing curve controllable with `notify_every_n_secs` & `after_every_n_notifications` that delivers alert notifications quickly at the start, then slows down notifications over time. 
